...
async def show_customer_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [KeyboardButton("✉️ Murojaat"), KeyboardButton("📘 Qo'llanma")],
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text(
...
async def handle_customer_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📘 Qo'llanma":
        await update.message.reply_text("Biz kagoo.uz jamoasi")
        return
            ...
            try:
                if phone and dishes_total:
                    ...
                    if cashback_earned > 0:
                        ...
                        # Generate a unique 6‑digit promo code if one does not exist
                        if not entry.get('promo'):
                            existing_codes = {e.get('promo') for e in cashbacks.values() if e.get('promo')}
                            import random
                            code = None
                            while True:
                                code_candidate = str(random.randint(100000, 999999))
        ...
        if customer_id:
            try:
                ...
                if cashback_earned > 0 and phone:
                    ...
                    if cb_entry:
                        message += f"\nSizga xaridingiz uchun {cashback_earned:,.0f} so'm keshbek berildi, sizda {cb_entry['balance']:,.0f} so'm jami keshbek bor uni keyingi xaridlaringiz uchun ishlatishingiz mumkin!"
                        if cb_entry.get('promo'):
                            message += f"\nKeshbekni ishlatish uchun {cb_entry['promo']} promo kodidan foydalaning!"
                await bot.send_message(
                    chat_id=customer_id,
                    text=message
                )
