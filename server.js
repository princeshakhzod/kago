...
const FASTAPI_ENDPOINT = process.env.FASTAPI_ENDPOINT || 'http://127.0.0.1:8000/api/orders';
const FASTAPI_AUTH_TOKEN = process.env.FASTAPI_AUTH_TOKEN || 'supersecret';
...
app.post('/api/agent', async (req, res) => {
  try {
    ...
    // server.js dagi /api/chat ichida:
    const apiKey = "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"; // API kalitingni bu yerga yoz

    if (!apiKey) {
      return res.status(500).json({ ok: false, error: 'OpenAI API key is not configured on the server' });
    }

    const systemPrompt = "Siz Kago AI yordamchisiz. Foydalanuvchiga qisqa va aniq javob bering.";
    ...
    const payload = JSON.stringify({ model: 'gpt-3.5-turbo', messages });
    const options = {
