...
const FASTAPI_ENDPOINT = 'http://bot:8000/api/orders';
...
app.post('/api/agent', async (req, res) => {
  try {
    ...
    // ---- KAGO konteksti (real ma'lumot) ----
    const restaurants = readData('restaurants.json');      // id, name, desc, location, active, branchTypeId ...
    const dishes      = readData('dishes.json');           // id, name, price, restaurant_id, variants, active ...
    const branchTimes = readData('branchtimes.json');      // { [restaurantId]: { start: "08:00", end: "23:00" } }
    const workhours   = readData('workhours.json');        // umumiy jadval (agar branchTimes yo'q bo'lsa fallback)

    // Yetkazib berish narxi qoidasi (misol sifatida — siz aytgandek):
    function calcDelivery(km) {
      if (km <= 3) return 10000;
      return 10000 + Math.ceil(km - 3) * 1800;
    }

    // Modelga yuboriladigan ixcham kontekst (token tejash uchun qisqa)
    const ctx = {
      restaurants: restaurants.map(r => ({ id: r.id, name: r.name, active: r.active })),
      dishes: dishes.filter(d => d.active !== false).slice(0, 200) // kerak bo'lsa cheklang
    ...
    // API kaliti shu fayl ichida saqlanadi. Agar mavjud bo'lmasa, xatolik qaytariladi.
    ...
    // Agar foydalanuvchi savoli Kago sayti va xizmatlari bilan bog'liq bo'lmasa,
    // oldindan belgilangan matn qaytariladi. Bu yerda saytga xos kalit so'zlar
    // ro'yxati keltirilgan. Savol boshqa mavzuda bo'lsa, javob berilmaydi.
    const siteKeywords = [
      'kago', 'sayt', 'narx', 'filial', 'filiallar', 'katalog', 'kategoriya',
      'delivery', 'yetkazib', 'manzil', 'adres', 'keshbek', 'cashback', 'promo',
      'promo kod', 'promokod', 'buyurtma', 'mahsulot', 'restoran', 'menu',
      'ichimlik', 'taom', 'to\'lov', 'aksiyalar', 'promo-kod', 'olish', 'yuborish'
    ];
    const lowered = input.toLowerCase();
    const matchesKeyword = siteKeywords.some(keyword => lowered.includes(keyword));
    if (!matchesKeyword) {
      // Foydalanuvchi boshqa mavzuda so'ragan. Sun'iy intellekt faqat sayt
      // bo'yicha yordam beradi.
      const replyMsg = 'Kechirasiz, men faqat Kago sayti va uning xizmatlari haqida maʼlumot bera olaman. Men Kago sunʼiy intellektiman va sizga mahsulot tanlash, narxlar, filiallar, yetkazib berish va boshqa shu kabi savollarda yordam beraman.';
      return res.json({ ok: true, reply: replyMsg });
    }

    // Kago AI uchun tizim prompti. Bu chatGPTga qanday javob berish kerakligini
    // tushuntiradi: foydalanuvchidan kelgan savollarga sayt doirasida
    // to'liq va to'g'ri ma'lumotlarni bera olishi, boshqa mavzudagi savollarga
    // javob bermasligi kerak.
    const systemPrompt = [
      "Siz Kago sun'iy intellektisiz.",
      "Sizga faqat Kago veb‑sayti va uning xizmatlari haqida savollar beriladi.",
      "Foydalanuvchilarning savollariga qisqa, aniq va to'liq maʼlumot bilan javob bering.",
      "Filiallar, kategoriyalar, mahsulotlar, narxlar, yetkazib berish, manzillar, keshbek tizimi, promo kodlar, sayt orqali buyurtma berish va kerakli mahsulotni topish kabi savollarga javob bering.",
      "Agar savol sayt yoki uning xizmatlari bilan bog'liq bo'lmasa, \"Kechirasiz, men faqat Kago sayti va uning xizmatlari haqida maʼlumot bera olaman\" deb javob bering va boshqa mavzuga javob bermang."
    ].join(' ');
    ...
    const messages = [
      {
        ...
        Yetkazib berish siyosati: 0–3 km = 10 000 so‘m; har qo‘shimcha km = +1 800 so‘m. \
        Foydalanuvchi aniq filial yoki taom so‘rasa, tegishlisini topib, nomi va narxi bilan qaytaring. \
        Dori/med. maslahat yoki KAGO bilan bog‘liq bo‘lmagan mavzularni muloyim rad eting va KAGO mavzulariga yo‘naltiring. \
        Maslahat berishda 2–3 varianti solishtirib bering; savol bo‘lsa qayta so‘rang, lekin ortiqcha so‘ramang."
      },
